...
// codexConfigModelRe matches a top-level `model = "..."` line in config.toml.
...
// missing/unreadable. An empty return means "let the CLI apply its own
// default" — buildArgs omits -m entirely in that case, so cogos never bakes
// in a hardcoded model id that drifts out of sync with the CLI's own choice
// (#627).
